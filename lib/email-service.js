...
export async function sendProjectInquiryEmail(formData) {
    try {
        ...
        console.log('Initiating project inquiry email send to:', emailUser);
        ...
        await transporter.sendMail(mailOptions);
        console.log('Project inquiry email sent successfully to:', recipients.join(', '));
        return { success: true };
    } catch (error) {
...
export async function sendChatbotLeadEmail(leadData) {
    try {
        ...
        const mailOptions = {
            from: `"ChittorTech AI Bot" <${emailUser}>`,
            to: recipients,
...
export async function sendInternshipApplicationEmail(formData) {
    try {
        const emailUser = process.env.EMAIL_USER?.trim();
        const emailPass = process.env.EMAIL_PASS?.trim().replace(/\s/g, '');
        if (!emailUser || !emailPass) return { success: false, error: 'Credentials missing' };
