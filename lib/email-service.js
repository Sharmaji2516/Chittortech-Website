/**
 * Dispatch emails using standard Resend REST API (Native Fetch).
 * Works seamlessly across both Server Node.js and Client Browser Static Exports.
 */
export async function dispatchEmail({ from, to, subject, html, attachments = [] }) {
  ...
  try {
    ...
    if (!response.ok || resData.error || resData.statusCode >= 400) {
      const errMsg = resData.error?.message || resData.message || resData.name || 'Resend send failed';
    ...
    return { success: false, error: err.message || 'Unable to fetch data' };
  }
}
