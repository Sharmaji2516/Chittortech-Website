...
export async function sendProjectInquiryEmail(formData) {
  ...
  // Single API Call: Delivers to Kush & Lav AND triggers 1 automatic Auto-Reply via linked template
  try {
...
export async function sendChatbotLeadEmail(leadData) {
  ...
  // Send Admin Notification to Kush & Lav ONLY (No Auto-Reply for Chatbot)
  try {
    await triggerEmailJS(params, ADMIN_TEMPLATE_ID);
  } catch (error) {
