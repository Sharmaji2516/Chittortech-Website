...
export async function sendOtpEmail(email, otp, auditData = null) {
    try {
        ...
        let actionLabel = "Infrastructure Access";
        if (auditData?.action === 'DATA_MODIFICATION') {
            actionLabel = "Editing/Adding Project";
        } else if (auditData?.action === 'SIGNUP') {
            actionLabel = "Admin Signup Request";
        } else if (auditData?.action === 'LOGIN') {
            actionLabel = "Admin Login";
        } else if (auditData?.action === 'VIEW') {
            actionLabel = "Viewing Project";
        }

        const subjectAction = auditData?.action || "Access";
