...
// Send dedicated individual email to each admin inbox
async function sendAdminNotifications(params) {
  return Promise.all(
    ADMIN_EMAILS.map(adminEmail =>
      triggerEmailJS({ ...params, to_email: adminEmail }, ADMIN_TEMPLATE_ID)
    )
  );
}
...
export async function sendProjectInquiryEmail(formData) {
  ...
  // 2. Send Admin Notification to EVERY admin inbox individually
  try {
    await sendAdminNotifications(params);
  } catch (emailError) {
...
export async function sendChatbotLeadEmail(leadData) {
  ...
  // Send Admin Notification to all 3 admins
  try {
    await sendAdminNotifications(params);
  } catch (error) {
