  ...
  useEffect(() => {
    ...
    class Particle {
      constructor() {
        ...
        this.vx = (Math.random() - 0.5) * 1.2; // Favor horizontal
        this.vy = (Math.random() - 0.5) * 0.3; // Less vertical
      ...
      draw() {
        ...
        ctx.fillStyle = 'rgba(0, 62, 216, 0.25)';
        ctx.fill();
      }
    }
        ...
        particles.forEach((p, index) => {
          ...
          for (let j = index + 1; j < particles.length; j++) {
            ...
            if (dist < connectionDistance) {
              ctx.beginPath();
              ctx.strokeStyle = `rgba(0, 62, 216, ${0.12 * (1 - dist / connectionDistance)})`;
              ctx.lineWidth = 1;
              ctx.moveTo(p.x, p.y);
